...
int al_contains(ArrayList* this, void* pElement)
{
    ...
    if(this != NULL && pElement != NULL) // if the ArrayList and pElement exist..
    {
        returnAux = 0;
        length= al_len(this);
        for(index=0;index < length;index++)// check the ArrayList...
        {
 ...
 *                  - (0) if ok
 */
int al_sort(ArrayList* listado,int (*pFunc)(void*,void*), int order)
{
  void* elA;
  void* elB;
  void* elAux;
  int i;
  int j;
  // Cambiar estos void* por el tipo de dato que pide el programa
  int returnAux = -1;
  ...
  if (listado != NULL)
  {
    if (order == 0)
	{
		for (i = 0; i < al_len(listado) - 1; i++)
		{
		   elA = al_get(listado, i);
		   for (j = i+1; j < al_len(listado); j++)
		   {
			  elB = al_get(listado, j);
			  if (pFunc(elA, elB) == 1)
			  {
				elAux = elB;
				elB = elA;
...
/** \brief Genera una lista filtrada y la devuelve
 *
 * \param ArrayList* listIn LISTA ORIGINAL
 * \param int (*functionFilter)(void*) FUNCION QUE SE ENCARGA DE FILTRAR
 * \return listaFiltrada NUEVA LISTA FILTRADA
 *
 */
ArrayList* al_filter(ArrayList* listIn , int (*functionFilter)(void*))
{
    ...
    if(listIn != NULL && functionFilter != NULL)
    {
        ...
        for(i=0;i < length;i++)
        {
            ...
            {
                listaFiltrada->add(listaFiltrada,elemento);
            }
        }
